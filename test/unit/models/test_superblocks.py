...
def go_list_proposals():
    items = [
        ...
        {u'AbsoluteYesCount': 1000,
         ...
         u'DataString': u'[["proposal", {"end_epoch": 2122520400, "name": "fernandez-7625", "payment_address": "mocfCW5mUWLdcqphNxbcsVKjzqTgdKckW6", "payment_amount": 32.01, "start_epoch": 1474261086, "type": 1, "url": "https://zcore.cash"}]]',
...
def test_superblock_is_valid(superblock):
    ...
    assert orig.is_valid(zcored) is True

    # mess with payment amounts
    superblock.payment_amounts = '7|yyzx'
    assert superblock.is_valid(zcored) is False
    ...
    assert superblock.is_valid(zcored) is True
...
def test_deterministic_superblock_creation(go_list_proposals):
    ...
    sb = zcorelib.create_superblock(prop_list, 72000, budget_max=max_budget, sb_epoch_time=misc.now())
